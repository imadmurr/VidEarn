package murr.imad.myapplication.payouts.data.repository
    ...
    suspend fun getAllCryptoPayouts(user: User): Result<List<CryptoPayout>> {
        return try {
            val snapshot = firestore.collection(Constants.PAYMENTS)
                .document(user.id)
                .collection(Constants.CRYPTO_PAYMENTS)
                .orderBy("added_date", Query.Direction.ASCENDING)
    ...
    suspend fun cancelCryptoPayout(user: User, cryptoPayout: CryptoPayout): Result<Unit> {
        return try {
            val snapshot = firestore.collection(Constants.PAYMENTS)
                .document(user.id)
                .collection(Constants.CRYPTO_PAYMENTS)
                .whereEqualTo("id", cryptoPayout.id)
                .whereEqualTo("amount", cryptoPayout.amount)
                .whereEqualTo("added_date", cryptoPayout.added_date)
                .get()
                .await()
    ...
    suspend fun cancelGiftCardPayout(user: User, giftCardPayout: GiftCardPayout): Result<Unit> {
        return try {
            val snapshot = firestore.collection(Constants.PAYMENTS)
                .document(user.id)
                .collection(Constants.GIFT_CARDS_PAYMENTS)
                .whereEqualTo("id", giftCardPayout.id)
                .whereEqualTo("amount", giftCardPayout.ptsAmount)
                .whereEqualTo("added_date", giftCardPayout.added_date)
                .get()
                .await()
